...
abstract public class AWindow {
	...
	protected int viewportX=0, viewportY=0, viewportWidth, viewportHeight;
	 ...
	 * */
	public AWindow(int wid, int hei, String title, Scene scene) {
	...
	protected Pair<Integer, Integer> getBufferSize() {
		try ( MemoryStack stack = stackPush() ) {
			IntBuffer pWidth = stack.mallocInt(1); // int*
			IntBuffer pHeight = stack.mallocInt(1); // int*
			glfwGetWindowSize(WINDOW_HANDLE, pWidth, pHeight);
